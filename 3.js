...
import { Button, message, Modal, Popconfirm, Select, Space, Tree, Upload } from 'antd';
...
const SysConfig: React.FC = () => {
  ...
  // 删除按钮状态
  const [deletListButton, setDeletListButton] = useState(true);
      ...
      render: (url: any) => {
        return (
          <Image
            ...
            src={url}
            fallback={
              'http://192.168.1.251:10015/store-management/static/goodsDefault.73f3d576.jpg'
            }
          />
        );
      },
  ...
  return (
    <PageContent>
      <Row justify="center" align="top">
        ...
        <Col flex="auto">
          <ProTable
            ...
            toolBarRender={() => [
              <Space>
                <Button icon={<FolderOpenOutlined />}>导出</Button>
                <Button
                  key="add"
                  onClick={() => {
      ...
      >
        <p>是否删除这{deleteList.length}条数据？</p>
      </Modal>
      <ModalForm
        className="ant-css"
        title="新增"
